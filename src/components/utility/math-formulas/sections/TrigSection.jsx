    ...
    <div className="mf-section" id="trig">
      ...
      <Card color="teal" style={{marginBottom:16}}>
        ...
        <div style={{display:'flex', flexWrap:'wrap', gap:12, margin:'12px 0'}}>
          <div style={{flex:1, minWidth:140, background:'color-mix(in srgb, var(--mf-teal) 8%, transparent)', borderRadius:12, padding:'12px 14px', borderLeft:'3px solid var(--mf-teal)'}}>
            <div style={{fontWeight:700, color:'var(--mf-teal)', marginBottom:10, fontSize:13}}>sin — বাড়তি ক্রম ↑</div>
            <div style={{display:'flex', alignItems:'flex-end', gap:10, marginBottom:8}}>
              {['√0','√1','√2','√3','√4'].map((v,i) => (
                <div key={i} style={{display:'flex', flexDirection:'column', alignItems:'center', gap:2}}>
                  <span style={{fontWeight:800, color:'var(--mf-teal)', fontSize:16, fontFamily:'var(--font-display)'}}>{v}</span>
          ...
          <div style={{flex:1, minWidth:140, background:'color-mix(in srgb, var(--mf-blue) 8%, transparent)', borderRadius:12, padding:'12px 14px', borderLeft:'3px solid var(--mf-blue)'}}>
            <div style={{fontWeight:700, color:'var(--mf-blue)', marginBottom:10, fontSize:13}}>cos — কমতি ক্রম ↓</div>
            <div style={{display:'flex', alignItems:'flex-end', gap:10, marginBottom:8}}>
              {['√4','√3','√2','√1','√0'].map((v,i) => (
                <div key={i} style={{display:'flex', flexDirection:'column', alignItems:'center', gap:2}}>
                  <span style={{fontWeight:800, color:'var(--mf-blue)', fontSize:16, fontFamily:'var(--font-display)'}}>{v}</span>
                  <span style={{width:'100%', height:1, background:'var(--text-dim)'}} />
                  <span style={{fontSize:12, color:'var(--text-3)', fontWeight:600}}>2</span>
                </div>
              ))}
          ...
          <div style={{flex:1, minWidth:140, background:'color-mix(in srgb, var(--mf-rose) 8%, transparent)', borderRadius:12, padding:'12px 14px', borderLeft:'3px solid var(--mf-rose)'}}>
            ...
            <div style={{display:'flex', alignItems:'flex-start', gap:10, marginBottom:8}}>
              {[['0°','√0'],['30°','√1'],['45°','√3'],['60°','√9']].map(([ang,v],i) => (
                <div key={i} style={{display:'flex', flexDirection:'column', alignItems:'center', gap:2}}>
                  ...
                  <span style={{fontWeight:800, color:'var(--mf-rose)', fontSize:16, fontFamily:'var(--font-display)'}}>{v}</span>
                  <span style={{width:'100%', height:1, background:'var(--text-dim)'}} />
              ...
              <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:2}}>
                ...
                <span style={{fontWeight:800, color:'var(--mf-rose)', fontSize:16, fontFamily:'var(--font-display)'}}>∞</span>
              </div>
            </div>
      ...
      <Card color="violet">
        ...
        <div className="mf-table-scroll">
          <table className="mf-cmp-table mf-no-cover" style={{minWidth:320}}>
            <thead>
              <tr><th>কোণ</th><th style={{color:'var(--mf-teal)'}}>sin</th><th style={{color:'var(--mf-blue)'}}>cos</th><th style={{color:'var(--mf-rose)'}}>tan</th></tr>
            </thead>
            <tbody>
              {[
                ['0°',  '0',    '1',      '0'],

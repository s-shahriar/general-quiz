  ...
  return (
    <div className="nailed-screen anim-fade">
      <div className="nailed-screen-topbar">
        <button className="back-btn" onClick={onHome}><ChevronLeft size={15} /> Back</button>
      ...
      {total === 0 ? (
        <div className="nailed-screen-empty">
          ...
          <span>Answer questions in Quiz, Study, or Exam mode and tap <strong>Important</strong> to save them here.</span>
        </div>
      ) : (
        <>
          ...
          <div className="nailed-screen-hint">
            Tap <X size={11} style={{ display: 'inline', verticalAlign: 'middle' }} /> to remove from Important.
          </div>
        ...
        <div className="nailed-group-body anim-slide">
          {items.map(({ q, qid }) => (
            <div key={qid} className="nailed-row">
              ...
              <button className="nailed-unnail-btn" onClick={() => onUnmark(qid)} title="Remove">
                <X size={13} />
              </button>
            </div>
          ))}

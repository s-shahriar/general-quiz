...
export function TrashProvider({ children }) {
  ...
  const [pending, setPending] = useState(null)   // { q, onDone } awaiting delete confirm
  ...
  // onDone runs after a successful trash — lets single-question modes advance to
  // the next question. List views instead just re-render (trashedIds hides the row).
  ...
  // Used by the Recycle Bin screen. Restore un-hides + re-fetches that module so
  // the question reappears; purge removes it for good.
      ...
      {pending && (
        <div className="trash-modal-backdrop" onClick={() => !busy && setPending(null)}>
          <div className="trash-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="trash-modal-preview"><RichText html={pending.q.question} /></div>

...
// Build a flat ordered list of {id, len} from the topic array
function buildTopicList(topics) {
  return topics.map(t => ({ id: t.id, len: t.questions.length }))
}

// Encode a Set of qids ("topicId__idx") into a compact Uint8Array bitmask buffer.
// Format: [numEntries:1] ([tIdx:1][byteLen:1][bitmask:byteLen])...
...
// topics = ALL_TOPICS array (needed to build the index)
